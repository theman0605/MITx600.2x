...
def r_squared(y, estimated):
    ...
    yMean = np.mean(y)
    yArray = np.array(y)
    estimatedArray = np.array(estimated)
    numerator = np.sum((yArray - estimatedArray)**2)
    denominator = np.sum((yArray - yMean)**2)
...
def evaluate_models_on_training(x, y, models):
    ...
    x = np.array(x)
    y = np.array(y)
    yEst = ((models[0][0] * x) + models[0][1])
    rsq = r_squared(y, yEst)
    pylab.plot(x, y, 'bo', label='Data')
    pylab.plot(x, yEst, 'r-', label='Model')
    pylab.title('Model Degree: %s, r^2 %f' % ((len(models), rsq)))
    pylab.xlabel('Year')
